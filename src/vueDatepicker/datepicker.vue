  ...
  <section class="date_container">
    ...
    <div class="date_content_hms" v-if="showhms">
      ...
      <div class="date_content_hms_list">
        <ul v-if="hmscon.hstatus">
          <li v-for="(ele,index) in hmscon.hours" @click.stop="selhms(index,'h')">{{index>9?index:"0"+index}}</li>
        </ul>
        <ul v-if="hmscon.mstatus">
          <li v-for="(ele,index) in hmscon.minutes" @click.stop="selhms(index,'m')">{{index>9?index:"0"+index}}</li>
        </ul>
        <ul v-if="hmscon.sstatus">
          <li v-for="(ele,index) in hmscon.seconds" @click.stop="selhms(index,'s')">{{index>9?index:"0"+index}}</li>
        </ul>
      </div>
    </div>
  </section>
...
::-webkit-scrollbar-thumb {
  ...
  .date_content:first-child .date_content_item {
    color: #ddd;
    font-size: 16px;
    height: 15%;
    line-height: 15%;
  ...
  .date_top_changge .date_content_li {
    cursor: auto;
    flex: 2.5;
  }
  ...
  .date_jiantou1:active,
  .date_jiantou2:active {
    border-color: #ddd;
  }

  .date_content_item div .date_isToday {
    margin: auto;
  ...
  .date_content_item div .date_isMark {
    margin: auto;
    border-radius: 100px;
  ...
  .date_content_item div .date_nextDayShow {
    color: #bfbfbf;
  }
  .date_content_hms{
    /*width:100%;*/
  ...
  .date_content_hms_tit{
    display:flex;
    flex-direction:row;
  }
  .date_content_hms_tit ul{
      flex:1;
      width:100%;
      height:60px;
      overflow-y: auto;
  ...
  .date_content_hms_tit ul li{
    line-height: 25px;
  }

  .date_content_hms_list{
    display:flex;
    flex-direction:row;
  }
  .date_content_hms_list ul{
      flex:1;
      width:100%;
      height:200px;
      overflow-y: auto;
  ...
  .date_content_hms_list ul li{
    line-height: 25px;
  }
  .date_content_hms_list ul li:hover{
